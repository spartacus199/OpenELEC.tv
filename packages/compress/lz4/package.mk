...
make_host() {
  make lib lz4programs
}

makeinstall_host() {
  mkdir -p $ROOT/$TOOLCHAIN/bin
    cp programs/lz4 $ROOT/$TOOLCHAIN/bin
    ln -sf lz4 $ROOT/$TOOLCHAIN/bin/lz4c
  mkdir -p $ROOT/$TOOLCHAIN/include
    cp lib/lz4.h $ROOT/$TOOLCHAIN/include
    cp lib/lz4hc.h $ROOT/$TOOLCHAIN/include
    cp lib/lz4frame.h $ROOT/$TOOLCHAIN/include
  mkdir -p $ROOT/$TOOLCHAIN/lib
    cp -P lib/liblz4.so* $ROOT/$TOOLCHAIN/lib
}
